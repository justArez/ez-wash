...
export default function AdminTierPage() {
  const [tierSets, setTierSets] = useState<TierSet[]>([]);
  ...
  // Active Set
  const activeSet =
    tierSets.find((s) => s.status === "Active") || tierSets[0] || null;
        ...
        <CardContent className="space-y-6 pt-4">
          {loading ? (
            <div className="py-12 flex items-center justify-center text-sm text-gray-400">
              <RefreshCw className="animate-spin w-5 h-5 mr-2 text-[#3a46ed]" />
              Loading membership tier sets...
            </div>
          ) : tierSets.length === 0 ? (
                ...
                <div className="flex-1">
                  ...
                  <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center">
                    ...
                    <Button
                      ...
                      disabled={!activeSet || selectedSetId === activeSet.id}
                      className="whitespace-nowrap"
                    >
                      <CheckCircle2 className="w-4 h-4 mr-1.5" />
                      {activeSet && selectedSetId === activeSet.id
                        ? "Currently Active"
                        : "Set As Active"}
                    </Button>
                  </div>
                  <p className="text-xs text-gray-500 mt-2">
                    Note: Only one Tier Set is active globally across all
                    customer bookings and calculations.
                  </p>
                </div>
              </div>
