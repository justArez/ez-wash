import type { LoyaltyStore, Promotion } from "../models/loyalty.model";

function createId() {
  return `${Date.now()}-${Math.random().toString(36).slice(2, 10)}`;
}

export function getAllPromotions(store: LoyaltyStore) {
  return store.promotions;
}

export function createPromotion(store: LoyaltyStore, data: Partial<Promotion>) {
...
export function deletePromotion(store: LoyaltyStore, promotionId: string) {
