...
export function shouldRunMonthlyEvaluation(store, today) {
    const now = new Date((today ?? new Date().toISOString().split("T")[0]) + "T00:00:00Z");
...
export function getTier(tierId, store) {
    if (store?.tiers?.length) {
        const tier = store.tiers.find((item) => item.id === tierId && item.isActive);
        if (tier) {
            return tier;
        }
    }
...
export function getAllTiers(store) {
    return store?.tiers?.length ? store.tiers : Object.values(TIERS);
}
export function getAllowedBookingWindowDays(tierId, store) {
    return getTier(tierId, store).bookingWindowDays;
}
export function getNextBookingWindowDate(date, tierId, store) {
    const nextDate = new Date(date);
    nextDate.setDate(nextDate.getDate() + getAllowedBookingWindowDays(tierId, store));
    return nextDate.toISOString().split("T")[0];
}
export function getAppliedPerks(tierId, store) {
    return getTier(tierId, store).perks;
}
export function createTier(store, data) {
...
export function updateTier(store, tierId, data) {
    ...
    if (data.pointRate !== undefined)
        tier.pointRate = data.pointRate;
    if (data.perks !== undefined)
        tier.perks = data.perks;
    if (data.description !== undefined)
        tier.description = data.description;
    if (data.isActive !== undefined)
        tier.isActive = data.isActive;
    tier.updatedAt = new Date().toISOString();
    return tier;
}
export function deleteTier(store, tierId) {
...
export function evaluateCustomerTiers(store, today) {
    ...
    for (const customer of store.customers) {
        ...
        const visits = customer.bookingHistory.filter((booking) => booking.status === "confirmed").length;
